...
evutil_socket_t make_socket(int port) {
...
void on_request(evhttp_request* req, void* arg) {
    auto* ctx = static_cast<HandlerCtx*>(arg);
    ctx->router->handle(req, ctx->base);
}

void on_signal(evutil_socket_t, short, void* arg) {
...
void Server::listen(int port) {
    if (!router_) {
        throw std::runtime_error("call routes() before listen()");
    }
    ...
    try {
        for (int i = 0; i < n_io_; ++i) {
            ...
            t.thread = std::thread([base = t.base, i] {
                tl_base = base;
                CPPFLUX_LOG(Info) << "[io " << i << "] started"
                                  << " thread=" << std::this_thread::get_id()
                                  << " backend=" << event_base_get_method(base);
                event_base_loop(base, EVLOOP_NO_EXIT_ON_EMPTY);
                tl_base = nullptr;
                CPPFLUX_LOG(Info) << "[io " << i << "] stopped";
            });
        }
    } catch (...) {
        cleanup_io_threads();
    ...
    CPPFLUX_LOG(Info) << "listening :" << port << "  io_threads=" << n_io_
    ...
    CPPFLUX_LOG(Info) << "[main] shutting down...";
...
void Server::cleanup_io_threads() {
    ...
    for (auto& t : io_threads_) {
        delete static_cast<HandlerCtx*>(t.ctx);
        t.ctx = nullptr;
        if (t.http) {
            evhttp_free(t.http);
            t.http = nullptr;
